...
def download(HW_ID: int, PROBLEM_ID:int, root_dir: str):
            ...
            with requests.get(file_url, headers=HEADER_DATA) as file_r:
                ...
                try:
                    with zipfile.ZipFile(BytesIO(file_r.content)) as zip_ref:
                        zip_ref.extractall(f'{root_dir}/{(str(PROBLEM_ID) + "/") if PROBLEM_ID != 0 else ""}{std_id}')
                except zipfile.BadZipFile:
                    if PROBLEM_ID != 0:
                        os.makedirs(f'{root_dir}/{PROBLEM_ID}/{std_id}', exist_ok=True)
                    else:
                        os.makedirs(f'{root_dir}/{std_id}', exist_ok=True)
                    file_name = re.search(
                        r'filename="(.+)"', file_r.headers['Content-Disposition']).group(1)
                    with open(f'{root_dir}/{(str(PROBLEM_ID) + "/") if PROBLEM_ID != 0 else ""}{std_id}/{file_name}', 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('problem_id', type=int, default=0, nargs='?',
    ...
    download(args.hw_id, args.problem_id, args.hw_dir)
